...
parser.add_argument('-time', action="store", dest="wake_up", default="7:00 AM", required=True, help="The time you want to wake up. Available formats are 7:00 PM, '7:00 PM', 19:00 or 1900. If you use a space, you must put time in quotes - '7:00 PM' ")
...
def process_time_from_user(wake_time):
      try:
        return time.strftime("%H%M", time.strptime(wake_time, "%I%M%p")) 
      except:
        pass
      try:
...
def play_local_video():
  ...
  play_video(random.choice(video_list), 60)
